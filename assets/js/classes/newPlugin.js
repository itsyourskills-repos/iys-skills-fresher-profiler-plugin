...
const imagePath =
  "https://cdn.jsdelivr.net/gh/itsyourskills-repos/iys-skills-fresher-profiler-plugin@main/assets/img/";
...
class IysSearchPlugin {
  ...
  searchAPI(searchValue,selectedValue,skillId) {
    ...
    if (isLoginUser && this.searchValue.length > 0) {
      let authApiUrl = `https://api.myskillsplus.com/api-search/?q=${encodedSearchValue}&asso_key=AF`;
      if (skillId) {
...
class IysFunctionalAreasPlugin extends IysSearchPlugin {
  ...
  `;
    ...
    cardBodyDiv.style.boxShadow = "none";
  ...
  // Function to create a breadcrumb item
    ...
    onInputCleared(){
      const dropdownMenuSkills = document.getElementById("skills-horizontal-menu");
      dropdownMenuSkills.style.display = "block"; 
      const dropdownMenu = document.querySelector(".dropdown-menu");
      dropdownMenu.style.display = "none";
      this.fetchSkills("")
      openHomeTab();
    ...
    async fetchSkills(query) {
      ...
      const url = query 
          ? `https://lambdaapi.iysskillstech.com/latest/dev-api/search-af-category-skills/?q=${encodeURIComponent(query)}&limit=10&asso_key=AF`
          : `https://lambdaapi.iysskillstech.com/latest/dev-api/search-af-category-skills/?limit=10&asso_key=AF`;
    ...
    this.updatedBreadcrumbItem.forEach((breadcrumbItem, index) => {
      ...
      breadcrumbLink.addEventListener("click", async () => {
        ...
        skills.forEach(child => {
            ...
            console.log("Pills created for breadcrumb");
        });
