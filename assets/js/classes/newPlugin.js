...
class IysFunctionalAreasPlugin extends IysSearchPlugin {
  ...
  setupDiv() {
    ...
    //       fetch(`https://lambdaapi.iysskillstech.com/latest/dev-api/search-category-skills/?q=${encodeURIComponent(query)}&limit=10&asso_key=AF`,{ signal: controller.signal })
    //           .then(response => response.json())
    //           .then(response => {
    //               dropdownMenu.innerHTML = "";
    //               if (response.matches.length > 0) {
